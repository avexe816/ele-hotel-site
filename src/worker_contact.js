/**
 * ELE HOTEL — Cloudflare Pages advanced-mode worker.
 *
 * Serves the static site and handles the contact form:
 *   POST /api/contact  ->  sends an e-mail to info@ele-hotel.com
 *
 * ---------------------------------------------------------------------------
 ...
 *       SMTP_USER   noreply@ele-hotel.com        Lark の「公共メールボックス」アドレス
 *       SMTP_PASS   Lark 管理画面で発行する IMAP/SMTP 専用パスワード
 *  B) Resend        RESEND_API_KEY
 *  C) Brevo         BREVO_API_KEY
 *
 * 共通の任意設定:
 *       CONTACT_TO    既定 info@ele-hotel.com
 *       CONTACT_FROM  既定 "ELE HOTEL <SMTP_USER または noreply@ele-hotel.com>"
 * ---------------------------------------------------------------------------
 */
...
function buildMail(d) {
  ...
  const text =
    "ELE HOTEL 公式サイトのお問い合わせフォームから送信されました。\n\n" +
  ...
  const html =
    ...
    '<p style="margin:0 0 16px">ELE HOTEL 公式サイトのお問い合わせフォームから送信されました。</p>' +
  ...
  const subject = `【ELE HOTEL お問い合わせ】${d.kind || "その他"} / ${d.name}`;
...
function mimeMessage({ fromName, fromAddr, to, replyTo, replyName, subject, text, html }) {
  ...
  const bd = "ele" + Math.random().toString(36).slice(2, 12);
  const head = [
    ...
    `Message-ID: <${crypto.randomUUID()}@ele-hotel.com>`,
    "MIME-Version: 1.0",
...
async function sendSmtp(env, d) {
  ...
  try {
    await say(null, [220]);
    await say("EHLO ele-hotel.com", [250]);
